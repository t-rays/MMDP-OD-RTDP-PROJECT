...
$Runs = @(
    @{
        Name = "Easy - empty-8-8"
        Manifest = "experiment_manifest_easy_unconstrained.json"
    },
    @{
        Name = "Medium - maze-32-32-2"
        Manifest = "experiment_manifest_medium_unconstrained.json"
    },
    @{
        Name = "Hard - warehouse-10-20-10-2-1"
        Manifest = "experiment_manifest_hard_unconstrained.json"
    }
)
...
$LogFile = Join-Path "results" "all_unconstrained_$Timestamp.log"
...
try {
    ...
    foreach ($Run in $Runs) {
        ...
        & $Python "src/run_isolated_matrix.py" $Run.Manifest "--overwrite"
