...
IMPLEMENTATION_VERSION = "lrtdp-solved-stopping-v8"
...
def _planning_config_for_seed(
    ...
    *,
    ...
    if mode in {"time", "time_memory"} and time_limit is None:
        raise ValueError(
            f"No time limit configured for {map_name}, agents={n_agents}"
        )
    if mode in {"memory", "time_memory"} and memory_limit is None:
        raise ValueError(
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "automatically by unconstrained and memory resource modes."
        ),
    )
    parser.add_argument(
